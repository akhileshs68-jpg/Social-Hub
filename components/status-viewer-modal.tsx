...
export function StatusViewerModal({ open, onOpenChange, statusGroup }: StatusViewerModalProps) {
  ...
  const { markStatusAsSeen } = useStatus()
  ...
  useEffect(() => {
    if (open && currentStatus && user) {
      markStatusAsSeen(currentStatus.id, user.piUid).catch((error) => {
        console.error("[socialhub] Failed to mark status as seen:", error)
      })
    }
  }, [open, currentStatus, user, markStatusAsSeen])
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Avatar className="h-10 w-10 border-2 border-white">
                  {currentStatus.authorPhotoURL && <AvatarImage src={currentStatus.authorPhotoURL} alt={statusGroup.username} />}
